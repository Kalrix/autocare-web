...
function CreateStorePage() {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        const fetchData = async ()=>{
            const { data: tasks } = await supabase.from("task_types").select("*").eq(storeType === "hub" ? "allowed_in_hub" : "allowed_in_garage", true);
            setTaskTypes(tasks || []);
            if (storeType === "garage") {
                const { data: hubs } = await supabase.from("store_admin").select("id, name").eq("type", "hub");
                setHubOptions(hubs || []);
            }
        };
        fetchData();
    ...
    const handleSubmit = async ()=>{
        const { data: insertedStore, error: storeError } = await supabase.from("store_admin").insert([
            {
                ...storeDetails,
                type: storeType
            }
        ...
        await supabase.from("store_task_capacities").insert(taskPayload);
        if (storeType === "garage" && selectedHubs.length > 0) {
            const tags = selectedHubs.map((hubId)=>({
                    garage_id: insertedStore.id,
                    hub_id: hubId
                }));
            await supabase.from("garage_hub_tags").insert(tags);
        }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b035fb7f._.js.map